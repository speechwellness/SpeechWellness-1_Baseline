...
from combine_dataset import CombineDataset, collate_fn

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--task", type=str, default="0")
    parser.add_argument("--mode", type=str, choices=["s", "t", "s+t"], default="s+t")
    parser.add_argument("--train_json", type=str, default="data/train_data.json")
    parser.add_argument("--dev_json", type=str, default="data/dev_data.json")
    parser.add_argument("--output_dir", type=str, default="results/task0/combine")
    parser.add_argument("--speech_file", type=str, default="results/task0/xlsr53/lr1e-4/embeddings.npy",)
    parser.add_argument("--text_file", type=str, default="results/task0/bert/lr2e-5/embeddings.npy")
    parser.add_argument("--speech_input_dim", type=int, default=1024)
    ...
    parser.add_argument("--dropout", type=float, default=0.5, help="Dropout to use.")
...
def main():
    ...
    os.system("cp {} {}".format("script/combine/combine_train.py", os.path.join(args.output_dir, "train.py")))
    os.system("cp {} {}".format("script/combine/combine_model.py", os.path.join(args.output_dir, "model.py")))

    speech_fea_dic = np.load(args.speech_file, allow_pickle=True).item()
    text_fea_dic = np.load(args.text_file, allow_pickle=True).item()
    train_dataset = CombineDataset(args.train_json, speech_fea_dic, text_fea_dic)
    dev_dataset = CombineDataset(args.dev_json, speech_fea_dic, text_fea_dic)
    ...
    model = ConcatModel(args).to(device)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            logits = model(
                batch["speech_feature"].to(device),
                batch["text_feature"].to(device),
            )
